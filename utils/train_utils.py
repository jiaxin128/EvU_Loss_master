...
class train_utils(object):
    ...
    def setup(self):
        ...
        self.datasets['test'] = Mechanical_datasets(args.data_dir, args.class_prior, args.normlizetype).data_preprare(
            'test')
        self.datasets['ood'] = Mechanical_datasets(args.data_dir, args.class_prior, args.normlizetype).data_preprare('ood')
    ...
    def validate(self, epoch):
        self.model.eval()
        test_pred, test_prob, test_labels, test_un = self.evaluate(self.dataloaders['val'], watermark=None)
        accuracy_test = accuracy_score(test_labels, test_pred)
        logging.info('Val: -Accuracy: {:.2%}'.format(accuracy_test))
