...
class AUCEvULoss(nn.Module):

    def __init__(self, beta=1):
        super(AUCEvULoss, self).__init__()
        self.beta = beta
        self.eps = 1e-10
    ...
    def forward(self, output, target, num_classes):
        ...
        for t in th_list:
            ...
            n_ac = torch.zeros(1, device=output.device)  # number of samples accurate and certain
            n_ic = torch.zeros(1, device=output.device)  # number of samples inaccurate and certain
            n_au = torch.zeros(1, device=output.device)  # number of samples accurate and uncertain
            n_iu = torch.zeros(1, device=output.device)  # number of samples inaccurate and uncertain

            activate = torch.nn.Softplus()

            for i in range(len(target)):
                if ((target[i].item() == predictions[i].item())
                        and unc[i].item() <= unc_th.item()):
                    """ accurate and certain """
                    n_ac += max_alpha[i] * (1 - activate(unc[i]))  #torch.tanh
                elif ((target[i].item() == predictions[i].item())
                      and unc[i].item() > unc_th.item()):
                    """ accurate and uncertain """
                    n_au += max_alpha[i] * activate(unc[i])
                elif ((target[i].item() != predictions[i].item())
                      and unc[i].item() <= unc_th.item()):
                    """ inaccurate and certain """
                    n_ic += (1 - max_alpha[i]) * (1 - activate(unc[i]))
                elif ((target[i].item() != predictions[i].item())
                      and unc[i].item() > unc_th.item()):
                    """ inaccurate and uncertain """
                    n_iu += (1 - max_alpha[i]) * activate(unc[i])
        ...
        auc_loss = -1 * self.beta * torch.log(auc_evu + self.eps)
        return auc_loss
