...
class AddSNRGaussian(object):
    def __init__(self, SNR=-5.0):
        self.SNR = SNR

    def __call__(self, seq):
        seq_len = seq.shape[1]
        # print(seq_len)
        random_noise = np.random.randn(seq_len)
        signal_power = np.sum(seq * seq) / seq_len
        noise_power = np.sum(random_noise * random_noise) / seq_len
        noise_variance = signal_power / (math.pow(10., (self.SNR / 10)))
        noise = (math.sqrt(noise_variance / noise_power) * random_noise)
        return seq + noise
